...
public:
    ...
    double optimized_histogram(cv::Mat image, bool show = false) {
        ...
        if (show) {
            if (stopline) {
                cv::putText(image, "Stopline detected!", cv::Point(static_cast<int>(w * 0.1), static_cast<int>(h * 0.1)), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(255, 255, 255), 1, cv::LINE_AA);
            }
            if (dotted) {
...
private:
    ...
    double total;
    cv::Mat image; //maskh, masks, maskd;
